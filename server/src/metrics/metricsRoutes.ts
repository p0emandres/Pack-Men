...
export async function metricsRoutes(fastify: FastifyInstance) {
   ...
   * GET /metrics
   * 
   * Fetches aggregated player metrics for the authenticated user.
   * 
   * Security: JWT validation ensures only authenticated users can access their own metrics.
   ...
   * - games_played: Sum of all games_played metric values
   * - games_won: Sum of all games_won metric values
   * - games_lost: Sum of all games_lost metric values
   * - total_sales: Sum of all total_sales metric values
   * - tasks_completed: Sum of all tasks_completed metric values
   */
  fastify.get(
    '/',
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        // Aggregate metrics by metric_name
        ...
        fastify.log.error({ err: error }, 'Metrics fetch error')
        reply.code(500).send({
